...
(function () {
  ...
  if (!window.__nukeTrackerLoaded) {
    ...
    Worker.prototype.postMessage = function (data, transfer) {
      try {
        if (data && data.type === 'init' && data.gameStartInfo) {
          ...
          if (sz) { gameMapSize = sz; maybeSetMapDimensions(); }
    ...
    CanvasRenderingContext2D.prototype.setTransform = function (a, b, c, d, e, f) {
      if (
        ...
        lastTransform = { scale: a, tx: e, ty: f };
    ...
    function _drawOverlay() {
      ...
      nukeMap.forEach(function (nuke) { _drawNuke(ctx, nuke, scale, tx, ty); });
    }
    ...
    function _drawNuke(ctx, nuke, scale, tx, ty) {
      ...
      // Impact zone circles
      var radii = NUKE_RADII[nuke.unitType] || { inner: 12, outer: 30 };
      ctx.beginPath();
  ...
  function refreshAutoRenewList() {
    ...
    for (const [smallID, name] of autoRenewPlayers) {
      ...
      removeBtn.addEventListener('mouseover', function () { this.style.background = '#991b1b'; });
      removeBtn.addEventListener('mouseout',  function () { this.style.background = '#7f1d1d'; });
    ...
    el.addEvent = function (event) {
      ...
      if (event.allianceID !== undefined && event.focusID !== undefined) {
        ...
        if (autoRenewPlayers.has(smallID)) {
          if (renewBtn) { try { renewBtn.action(); } catch (_) {} }
          return orig({
            ...event,
          ...
          return orig({ ...event, buttons: [...(event.buttons ?? []), autoBtn] });
        }
      }
  ...
  function watchForElement() {
    ...
    if (existing) { tryInitElement(existing); return; }
  ...
  function initGame() {
    ...
    for (const [ctor, callbacks] of eventBus.listeners) {
      if (callbacks.some(fn => {
        try { return fn.toString().includes('createStructure'); }
  ...
  function doPlace() {
    ...
    requestAnimationFrame(() => {
      if (!turboUnit) return;
      try { eventBus.emit(new MouseUpEventCtor(mouseX, mouseY)); }
      catch (err) { console.warn('[TurboPlace] emit error:', err); }
    });
  }

  function stopTurbo() {
    if (turboTimer !== null) { clearInterval(turboTimer); turboTimer = null; }
    turboUnit    = null;
    turboKeyCode = null;
  ...
  function setupUI() {
    ...
    _turboIntervalLbl.addEventListener('click', () => {
      ...
      Object.assign(input.style, {
        fontFamily: 'monospace', fontSize: '13px', width: '60px',
        textAlign: 'center', background: '#1f2937', color: '#f5a623',
        border: '1px solid #f5a623', borderRadius: '3px', padding: '1px 4px',
      });
      input.type = 'number';
      ...
      input.addEventListener('keydown', ev => {
        ...
        if (ev.key === 'Escape') { intervalRow.replaceChild(_turboIntervalLbl, input); }
        ev.stopPropagation();
      }, { capture: true });
    });
    ...
    tabEls.forEach((tabEl, i) => {
      tabEl.addEventListener('click', () => {
        tabEls.forEach(t => { t.className = 'of-tab'; });
        tabEl.className = 'of-tab of-tab-active';
        panes.forEach(p => { p.className = 'of-tab-pane'; });
  ...
  function setupSidebarButton(panel) {
    ...
    function attachToSidebar(sidebar) {
      ...
      if (!tryInject()) {
        ...
        waitObserver.observe(sidebar, { childList: true, subtree: true });
      }
    }
  ...
  function savePrefs() {
    try {
      localStorage.setItem(PREFS_KEY, JSON.stringify({
        nukeEnabled, autoRenewEnabled, turboEnabled, intervalMs,
      }));
    } catch (_) {}
  }
